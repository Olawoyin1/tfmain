...
import { FiClock, FiCreditCard, FiUsers, FiBook, FiBriefcase, FiAward } from 'react-icons/fi';
    ...
    <section className="process">
      <div className="process-inner">
            ...
            {steps.map((step, index) => (
              <div key={index} className="proc-step">
                <div className="flex flex-col items-center gap-0">
                  ...
                  {index < steps.length - 1 && <div className="proc-line"></div>}
                </div>
                <div className="proc-content">
                  <div className="proc-title">{step.title}</div>
                  <p className="proc-desc">{step.description}</p>
                  <span className="proc-note" style={{ display: 'inline-flex', alignItems: 'center', gap: '5px' }}>
                    {step.noteIcon} {step.note}
                  </span>
                </div>
              </div>
            ))}
        ...
        <div className="proc-timeline-dark">
          <div className="timeline-title">Your 6-Month Timeline</div>
            ...
            {timeline.map((item, index) => (
              <div key={index} className="timeline-item">
                ...
                <div className="timeline-content">
                  ...
                  <div className="timeline-text" dangerouslySetInnerHTML={{ __html: item.text }}></div>
                </div>
              </div>
            ))}
          </div>
